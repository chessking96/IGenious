...
def createAllSinglePlots():
    ...
    folder_names = ['simpsons', 'matmul', 'dot', 'DFT16dd', 'funarc', 'linear', 'newton_root']
                ...
                for vec in vectorized:
                    for tuning in tunings:
                        for folder in folder_names:
                            ...
                            precs_fix, times_fix = getFixedData('dd', input_range, vec, folder)
                            ...
                            plt.scatter(precs_d, times_d, c = colors, s = 60, label = 'double-double input precision')
                            colors = len(precs_dd) * ['green']
                            plt.scatter(precs_dd, times_dd, c = colors, s = 30, label = 'double input precision')
                            colors = len(precs_fix) * ['red']
                            plt.scatter(precs_fix, times_fix, c = colors, label = 'No mixed tuning')
                            #for i, txt in enumerate(names_sat_prec):
                            #    plt.annotate(txt, (precs_sat_prec[i], times_sat_prec[i]))
                            ...
                            plt.savefig('plots_new/dVSdd/' + file_name, bbox_inches='tight')
                            #plt.show()
                            plt.clf()
