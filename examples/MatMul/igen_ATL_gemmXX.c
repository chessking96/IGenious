...
void ATL_dGEMM2NN(const int M, const int N, const int K, f64_I alpha,
  ...
  {
    Kp = ((67108864 / sizeof(f64_I)) - 56 * 56) / (56 + 56);
    if (Kp > K || Kp < 56) {
      Kp = K;
    }
  }
      ...
      if (mm2(CblasNoTrans, CblasNoTrans, M, N, Kp, alpha, A, lda, B, ldb, bet,
        ...
        if (ATL_dmmJITcp(CblasNoTrans, CblasNoTrans, -M, N, Kp, alpha, A, lda,
          ...
          if (!(mmNC(CblasNoTrans, CblasNoTrans, M, N, Kp, alpha, A, lda, B,
                     ldb, bet, C, ldc) == 0)) {
            ATL_xerbla(0,
                       "/home/cgo2021/artifact/benchmarks/ATLAS_gemm_scalar/"
                       "atlas_source/__prep.c",
