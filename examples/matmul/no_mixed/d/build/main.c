#include "igen_lib.h"
#include "matmul.c"
#include "random_range.c"
#include <fenv.h>
#include <math.h>
#include <stdio.h>
#include <time.h>
int main() {
  fesetround(2048);
  initRandomSeed();
  f64_I *x_0 = malloc(10000 * sizeof(f64_I));
  for (int i = 0; i < 10000; i++) {
    f64_I h = getRandomDoubleInterval();
    x_0[i] = h;
  }
  f64_I *x_1 = malloc(10000 * sizeof(f64_I));
  for (int i = 0; i < 10000; i++) {
    f64_I h = getRandomDoubleInterval();
    x_1[i] = h;
  }
  f64_I *x_2 = malloc(10000 * sizeof(f64_I));
  for (int i = 0; i < 10000; i++) {
    f64_I h = getRandomDoubleInterval();
    x_2[i] = h;
  }
  clock_t start = clock();
  for (long i = 0; i < 20; i++) {
    matmul(x_0, x_1, x_2);
  }
  clock_t end = clock();
  long diff_time = (end - start);
  FILE *file = fopen("score.cov", "w");
  fprintf(file, "%ld\n", diff_time);
  fclose(file);
	int max = 0;
	int imax = 0;
	dd_I diff_max = _ia_zero_dd();
	for(int i = 0; i < 10000; i++){
		dd_I lower_bound = _ia_set_dd(x_2[i].lo, 0, -x_2[i].lo, -0);
		dd_I upper_bound = _ia_set_dd(-x_2[i].up, -0, x_2[i].up, 0);
		dd_I diff = _ia_sub_dd(upper_bound, lower_bound);
		if(_ia_cmpgt_dd(diff, diff_max)){
			diff_max = diff;
			max = i;
		}
	}
	char* answer = "false";
	double th = 100000;
	if((int)_ia_cmpgt_dd(_ia_set_dd(-th, 0, th, 0), diff_max) == 1){
		answer = "true";
	}
	file = fopen("sat.cov", "w");
	fprintf(file, "%s\n", answer);
	fclose(file);
	file = fopen("precision.cov", "w");
	double prec = _ia_cast_dd_to_f64(diff_max).up;
	fprintf(file, "%.17g\n", prec);
	printf("Precision constraint: %s\n", answer);
	printf("Diff lower bound: %.17g %.17g\n", diff_max.lh, diff_max.ll);
	printf("Diff upper bound: %.17g %.17g\n", diff_max.uh, diff_max.ul);

}
